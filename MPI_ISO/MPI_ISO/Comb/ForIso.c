...
int main(int argc, char **argv){
	...
	if(rank == 0){
		...
		for( i = 0; i < numtasks; i++){
			...
			/* Assign work for each processor */
			start = (int) floor((i * count)/numtasks);
			end = (int)floor(((i+1) * count)/numtasks); 
			
			/* Send the assigned work */
			MPI_Send(msg + (int) floor((i * count)/numtasks), (int)floor(((i+1) * count)/numtasks) - (int)floor((i * count)/numtasks)
